...
public class LongestPalindromeSubsequence {
    public ArrayList<Character> longestPalindromeSubsequence(ArrayList<Character> input) {
        ...
        // Base case: enkelt karakter = palindrome af længde 1
        for (int i = 0; i < n; i++) {
            dp[i][i] = 1;
        }
        ...
        while (i <= j) {
            if (i == j) {
                ...
                break;
            } else if (input.get(i).equals(input.get(j))) {
                result.add(input.get(i));
                i++;
                j--;
            } else {
        ...
        // opbyg fuld palindrom ved at mirror (bortset fra midten hvis ulige længde)
        int palindromeLength = dp[0][n - 1];
        for (int k = palindromeLength / 2 - 1; k >= 0; k--) {
