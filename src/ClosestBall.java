...
public class ClosestBall {
    public int computeClosest(ArrayList<Integer> players, ArrayList<Integer> balls) {
        //Sorterer begge liste
        Collections.sort(players);
        Collections.sort(balls);
        // definierer i,j og saetter mindste distance til infinity
        int i = 0; //tilhoerer player listen
        int j = 0; //tilhoerer ball listen
        int minDist = Integer.MAX_VALUE;

        // Mens i og j er mindre end længden af deres respektive lister
        while (i < players.size() && j < balls.size()) {
            // udregner distancen
            int dist = Math.abs(players.get(i) - balls.get(j));
            // opdater hvis mindre
            if (dist < minDist) {
                minDist = dist;
            }

            // hvis players position er mindre en boldens, tages den naeste player
            // ellers tages den naeste ball
