...
public class Inversions {
    public int countInversions(ArrayList<Integer> input) {
        ...
        int[] a = new int[n];
        for (int i = 0; i < n; i++) a[i] = input.get(i);
        int[] temp = new int[n];

        long inv = sortAndCount(a, temp, 0, n - 1);
        return (int) inv;
    }

    // Recursiv sortering af A og taeller antal inversioner
    private long sortAndCount(int[] a, int[] temp, int lo, int hi) {
        if (lo >= hi) return 0;
        int mid = lo + (hi - lo) / 2;
        // opdeler A i ved midten og tæller rekursivt antal inversioner i hver liste
        long leftInv  = sortAndCount(a, temp, lo, mid);
        long rightInv = sortAndCount(a, temp, mid + 1, hi);
        // tæller inversioner mellem de nyskabte lister
        long crossInv = mergeAndCount(a, temp, lo, mid, hi);

        return leftInv + rightInv + crossInv;
    }

    // Merger og taeller antal inversioner med et element fra hver liste
    private long mergeAndCount(int[] a, int[] temp, int lo, int mid, int hi) {
        int i = lo, j = mid + 1, k = lo;
        long inv = 0;

        while (i <= mid && j <= hi) {
            if (a[i] <= a[j]) {           // Samme vaerdi er ikke en inversion
                temp[k++] = a[i++];
            } else {
                temp[k++] = a[j++];
                inv += (mid - i + 1);     // Alle i left > a[j-1]
            }
        }
