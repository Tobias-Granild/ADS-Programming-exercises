...
public class ShortPath {
    public int shortestPath(Edge[] edges, int nodeCount, int maxEdges, int sourceNode, int targetNode) {
        ...
        int[] curr = new int[nodeCount];

        Arrays.fill(prev, INF);
        prev[sourceNode] = 0;

        for (int i = 1; i <= maxEdges; i++) {
            System.arraycopy(prev, 0, curr, 0, nodeCount);

            for (Edge e : edges) {
                if (prev[e.from] != INF) {
                    int newDist = prev[e.from] + e.weight;
                    if (newDist < curr[e.to]) {
                        curr[e.to] = newDist;
                    }
                }
            }
        ...
        return prev[targetNode] == INF ? Integer.MAX_VALUE : prev[targetNode];
    }
