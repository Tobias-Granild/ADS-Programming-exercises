...
public class MazePair {
    public int shortestPath(char[][] maze) {
        ...
        while(!queue.isEmpty()){
            ...
            //neighborloop
            for (int k = 0; k<4; k++){
                int nextR =  current.row + dr[k];
                int nextC = current.col + dc[k];
                if (nextR >= 0 && nextR < height && nextC >= 0 && nextC < width && maze[nextR][nextC] != 'o' && dist[nextR][nextC]== Integer.MAX_VALUE){
                    dist[nextR][nextC] = dist[current.row][current.col] + 1;
                    queue.add(new Coordinate(nextR, nextC));
                    if (maze[nextR][nextC] == 'b'){
                        mindist = Math.min(mindist, dist[nextR][nextC]);
                    }
                }
            }
        }
