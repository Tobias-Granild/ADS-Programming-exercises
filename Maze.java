...
// Contributions:
//   <Kristian Rasmussen> <Lavet i fællesskab>
//   <Kasper Frost> <Lavet i fællesskab>
//   <Tobias Granild> <Lavet i fællesskab>
...
public class Maze {
    public int shortestPath(char[][] maze) {
        ...
        int endR = height - 2, endC = width - 2;
        ...
        while (!queue.isEmpty()) {
            ...
            int row = cur[0], collumn = cur[1];

            if (row == endR && collumn == endC) {
                return dist[row][collumn];
            }

            for (int k = 0; k < 4; k++) {
                int newR = row + dr[k];
                int newC = collumn + dc[k];

                if (newR >= 0 && newR < height && newC >= 0 && newC < width &&
                maze[newR][newC] == '.' && dist[newR][newC] == -1) {
                    dist[newR][newC] = dist[row][collumn] + 1;
